...
var PanelManager = /** @class */ (function() {
  function PanelManager(mapApi) {
    ...
    this.panel.element.addClass(
      "ag-theme-material mobile-fullscreen tablet-fullscreen"
    );
  ...
  PanelManager.prototype.setListeners = function() {
    ...
    enhancedTable.on("keydown keyup", function(event) {
      ...
      var inList =
        $(document.activeElement).hasClass("rv-focus-list") ||
        $(document.activeElement)
          .parents()
          .hasClass("rv-focus-list");
      var focusedCell = that.tableOptions.api.getFocusedCell();
      if (
        focusedList !== undefined &&
        $(focusedList).hasClass("ag-body-container")
      ) {
      ...
      if (
        ...
        ($(".element-focused")[0] === undefined && inList)
      ) {
  ...
  PanelManager.prototype.prepListNavigation = function() {
    ...
    this.panel.populateList.subscribe(function() {
      ...
      panelBody
        .find(".ag-body-container")
        .addClass("rv-focus-list disabled-arrows");
      // add rv-focus-item class to header cells with content in it
      panelBody.find(".ag-header-cell").each(function(index, cell) {
        if (
          $(cell).children(
            ":not(span, .ag-cell-label-container, .ag-floating-filter-body)"
          ).length > 0
        ) {
  ...
  PanelManager.prototype.open = function(tableOptions, layer, tableBuilder) {
    ...
    if (this.currentTableLayer === layer) {
      ...
      this.panelStatusManager = new panel_status_manager_1.PanelStatusManager(
        this
      );
      ...
      this.makeHeader();
      this.panel.header.title =
        "{{ 'filter.title' | translate }} " + this.configManager.title;
      ...
      if (this.panelStateManager.columnState) {
        this.tableOptions.columnApi.setColumnState(
          this.panelStateManager.columnState
        );
      }
      ...
      this.tableOptions.onGridReady = function() {
        ...
        _this.gridBody = _this.panel.element[0].getElementsByClassName(
          "ag-body"
        )[0];
        _this.gridBody.tabIndex = 0; // make grid container tabable
        grid_accessibility_1.initAccessibilityListeners(
          _this.panel.element[0],
          _this.gridBody,
          _this.tableOptions
        );
        ...
        _this.tableOptions.columnDefs.forEach(function(column) {
          var matchingCol = _this.columnMenuCtrl.columnVisibilities.find(
            function(col) {
  ...
  PanelManager.prototype.cleanUp = function() {
    ...
    if (this.gridBody !== undefined) {
      grid_accessibility_1.removeAccessibilityListeners(
  ...
  PanelManager.prototype.onBtnExport = function() {
    var dataColumns = this.tableOptions.columnApi
  ...
  PanelManager.prototype.createHTMLTable = function() {
    ...
    var columns = {};
    this.tableOptions.columnApi.columnController.allDisplayedCenterVirtualColumns.map(
      function(col) {
        if (
          col.colDef.field !== "rvSymbol" &&
          col.colDef.field !== "rvInteractive" &&
          col.colDef.field !== "zoom"
        ) {
  ...
  PanelManager.prototype.makeHeader = function() {
    ...
    header.prepend(
      this.compileTemplate(templates_1.COLUMN_VISIBILITY_MENU_TEMPLATE)
    );
  ...
  PanelManager.prototype.hideToolTips = function() {
    Array.from(document.getElementsByTagName("md-tooltip")).forEach(function(
      tooltip
    ) {
  ...
  PanelManager.prototype.angularHeader = function() {
    var that = this;
    this.mapApi.agControllerRegister("ToastCtrl", [
      "$scope",
      "$mdToast",
      "$rootElement",
      function($scope, $mdToast, $rootElement) {
      ...
      this.clearFilters = function() {
        ...
        var preservedColumns = columns.map(function(column) {
          ...
          if (columnConfigManager.isFilterStatic) {
            newFilterModel[column] = that.tableOptions.api.getFilterModel()[
              column
            ];
            return column;
          }
      ...
      this.noActiveFilters = function() {
        if (that.tableOptions.api !== undefined) {
          ...
          var noFilters = !columns.some(function(col) {
            var columnConfigManager = new config_manager_1.ColumnConfigManager(
              that.configManager,
              col
            );
    ...
    this.mapApi.agControllerRegister("ApplyToMapCtrl", function() {
      ...
      function getFiltersQuery() {
        ...
        if (that.searchText && that.searchText.length > 2) {
          var globalSearchVal =
            globalSearchToSql() !== "" ? globalSearchToSql() : "1=2";
          if (globalSearchVal) {
        ...
        switch (colFilter.filterType) {
          case "text":
            if (column.isSelector) {
              return (
                "UPPER(" + col + ") IN (" + colFilter.filter.toUpperCase() + ")"
              );
            } else {
      ...
      function globalSearchToSql() {
        ...
        columns.forEach(function(column) {
          for (
            var _i = 0, sortedRows_1 = sortedRows;
            _i < sortedRows_1.length;
            _i++
          ) {
