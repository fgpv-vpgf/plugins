...
var PanelManager = /** @class */ (function () {
    function PanelManager(mapApi) {
        ...
        this.panel.closing.subscribe(function (response) {
            ...
            var mapNavContent = $('#' + mapApi.id).find('.rv-mapnav-content');
    ...
    PanelManager.prototype.setListeners = function () {
        ...
        var that = this;
        enhancedTable.mouseover(function () {
            $('.ag-body-viewport')[0].style.position = 'initial';
        });
        enhancedTable.mouseleave(function () {
            $('.ag-body-viewport')[0].style.position = 'initial';
        });
        enhancedTable.on('keydown', function (event) {
            var focusedList = $('.element-focused')[0];
            var inList = $(document.activeElement).hasClass('list') || $(document.activeElement).parents().hasClass('list');
            ...
            if (event.keyCode !== 9 || ($('.element-focused')[0] === undefined && inList)) {
                // if you are not tabbing or you are tabbing within a list
