...
var PanelManager = /** @class */ (function () {
    function PanelManager(mapApi) {
        ...
        var close = this.panel.header.getCloseButton();
        ;
    ...
    PanelManager.prototype.open = function (tableOptions, layer, tableBuilder) {
        ...
        if (this.currentTableLayer === layer) {
            ...
            this.makeHeader();
            this.panel.header.setTitle("Features: " + this.configManager.title);
            ...
            this.tableOptions.onGridReady = function () {
                ...
                var sourceEl = $(document).find("[legend-block-id=\"" + _this.legendBlock.id + "\"] button").filter(':visible').first();
    ...
    PanelManager.prototype.deleteLoaderPanel = function () {
        $('#enhancedTableLoader').remove();
        var loaderPanel = this.mapApi.panels.find(function (panel) { return panel.id === 'enhancedTableLoader'; });
        var loaderPanelIndex = this.mapApi.panels.indexOf(loaderPanel);
